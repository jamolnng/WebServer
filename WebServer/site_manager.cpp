...
SiteManager::SiteManager(const webserver::Config& conf) :
	config(conf)
{
	std::filesystem::path sites(config["sites"]);
	if (sites.is_relative())
		sites = config.getParent() / sites;
...
void SiteManager::load(const std::filesystem::path& path)
{
	...
	if (s.isDefault())
		defaultSite = &sites.find(s.getName())->second;
}

Site& SiteManager::get(const std::string& name)
{
	if(sites.find(name) != sites.end())
		return sites.find(name)->second;
	return getDefault();
}
...
{
	if (defaultSite == nullptr)
		return sites.begin()->second;
	return *defaultSite;
}